...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Received error report:", body.ticketNumber);

    const { error: dbError } = await supabase.from("maintenance_tickets").insert({
    ...
    console.log("Error saved successfully:", body.ticketNumber);
    return new Response(
